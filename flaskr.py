...
def exibir_entradas():
    ...
    cursor = g.br.execute(sql)
    entradas = [dict(titulo=titulo, texto=texto) for titulo, texto in cursos.fetchall()]
...
def login():
    ...
    if request.form['usuario'] != USERNAME or request.form['password'] != PASSWORD:
